...
import { GTM_ENTITLEMENT, GTM_PDF_SCOPE } from "@/features/marketing/constants";
...
/**
 * GET /api/marketing/:id/pdf — the branded A4 go-to-market report.
 *
 ...
 * workspace that has lost Marketing Intelligence should not keep a working
 * export URL for a document full of its own customer and pricing strategy.
 */
export const GET = withApiAuth<{ id: string }>(
  {
    route: "GET /api/marketing/:id/pdf",
    scope: GTM_PDF_SCOPE,
    errorMessage: "Could not export the go-to-market report.",
  },
  async ({ user, params: { id } }) => {
    ...
    const access = await canAccess(workspace.id, GTM_ENTITLEMENT);
    if (!access.allowed) {
      return apiError(
        "FORBIDDEN",
    ...
    const composed = await composeGtmReport(workspace.id, id);
    ...
    if (!("model" in composed)) {
      ...
      const notFound =
        composed.reason === "This marketing plan could not be found.";
      return apiError(
    ...
    return new Response(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${toPdfFilename(model.title, "aiautomix-gtm-plan")}"`,
